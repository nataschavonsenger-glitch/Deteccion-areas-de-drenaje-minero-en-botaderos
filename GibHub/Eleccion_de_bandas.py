    ...
    for i in range(retries):
        try:
            ...
            if "Too many concurrent aggregations" in str(e):
                ...
                raise
    raise RuntimeError("No se pudieron calcular metricass.")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if '429' in str(e) or 'Too many concurrent aggregations' in str(e):
                print(f" intento {attempt+1}/{max_retries} fallido. Reintentando en {wait}s…")
                time.sleep(wait)
                wait *= 2
            else:
                raise
...
def train_rf_random_search(
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for i in range(1, n_trials+1):
            ...
            print(f"\n Trial {i}/{n_trials} — size={subset_size}, bandas={subset}")
...
def main():
    ...
    fc2023 = ee.FeatureCollection(
        "projects/ee-projects/assets/2023"
    ).map(_add_label)
    fc2017 = ee.FeatureCollection(
        "projects/ee-projects/assets/2017"
    ).map(_add_label)
    fc2014 = ee.FeatureCollection(
        "projects/ee-projects/assets/20144"
    ).map(_add_label)
